...
async function showPosition(position){
    ...
    let response = await fetch('https://api.translink.ca/rttiapi/v1/stops?apikey=8dBFRSNBr4HQDO6mi9Ee&lat='+lat+'&long='+lon+'&radius=2000');
    let data = await response.text();
    ...
    for (let i = 0; i < queries.length; i++){
        ...
        setTimeout(() => {queries[i].style.opacity = 1; window.scrollTo(0, document.body.scrollHeight); }, i*100);
    }

    //data: atstreet, city, distance, name, on street, routes, stopno
}
